...
const PipelineValue = ({ subaccountId }: PipelineValueProps) => {
      ...
      return (
        pipelines
          ...
          ?.Lane?.reduce((totalLanes, lane, currentLaneIndex, array) => {
            const laneTicketTotal = lane.Tickets.reduce(
              (totalTickets, ticket) => totalTickets + Number(ticket.value),
              0
            );

            if (currentLaneIndex === array.length - 1) {
              setPipelineClosedValue(laneTicketTotal || 0);
              return totalLanes;
            }
            return totalLanes + laneTicketTotal;
          }, 0) || 0
      );
    }
